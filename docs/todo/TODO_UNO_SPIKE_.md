...
- The generated target is `net10.0-desktop` with `SimpleTheme` and `SkiaRenderer`.
...
- Direct project build passed:
  `dotnet build src\StoryWeaver.Uno\StoryWeaver.Uno\StoryWeaver.Uno.csproj -f net10.0-desktop`.
- Windows launch smoke passed: `dotnet run --project src\StoryWeaver.Uno\StoryWeaver.Uno\StoryWeaver.Uno.csproj -f net10.0-desktop --no-build`
...
- WSL build check passed on Ubuntu 26.04 under WSL2 using .NET SDK 10.0.400
  installed in `/home/pavel/.dotnet`.
- WSL build command:
...
- WSL build result: succeeded with one warning, zero errors. The warning is
...
- Runtime policy for a .NET 10 UI beside a .NET 8 engine.
