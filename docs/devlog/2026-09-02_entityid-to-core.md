# 2026-09-02 — EntityId moves, and the check finally runs on real saves
...
It was deferred earlier the same day with a stated trigger: **revisit when a second Core caller
needs to validate an id.** `CanonRefresh` became that caller within hours — a save hand-edited
...
hypothetical. **A trigger that fires the same day is a trigger that worked**, not one that was
set too far out.
