...
{
    public static IReadOnlyList<EntityDetails> Characters(WorldState world) => world.Characters
        .OrderBy(pair => pair.Value.Name, StringComparer.OrdinalIgnoreCase)
        .Select(pair => { var character = pair.Value; return new EntityDetails(
    ...
    public static IReadOnlyList<EntityDetails> Locations(WorldState world) => world.Locations
        .OrderBy(pair => pair.Value.Name, StringComparer.OrdinalIgnoreCase)
        .Select(pair => { var location = pair.Value; return new EntityDetails(
    ...
    public static IReadOnlyList<EntityDetails> Facts(WorldState world) => world.Facts
        .OrderBy(pair => pair.Value.EstablishedTurn).ThenBy(pair => pair.Value.Text, StringComparer.OrdinalIgnoreCase)
        .Select(pair => { var fact = pair.Value; return new EntityDetails(
    ...
    public static IReadOnlyList<EntityDetails> Items(WorldState world) => world.Items
        .OrderBy(pair => pair.Value.Name, StringComparer.OrdinalIgnoreCase)
        .Select(pair => { var item = pair.Value; return new EntityDetails(
